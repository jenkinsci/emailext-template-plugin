 ...
 */
@WithJenkins
class ExtEmailTemplateSlicerTest {
    ...
    void testAddTemplate() throws IOException {
        ...
        ExtendedEmailTemplatePublisher publisher =
                (ExtendedEmailTemplatePublisher) project.getPublishersList().get(0);
        ...
        for (final TemplateId id : ids) {
            if (firstId) {
                assertTrue(ID_TEMP1.equalsIgnoreCase(id.getTemplateId()));
                firstId = false;
            } else assertTrue(ID_TEMP2.equalsIgnoreCase(id.getTemplateId()));
        }
    }
    ...
    void testRemoveTemplate() throws IOException {
        ...
        for (final TemplateId id : ids) {
            if (firstId) {
                ...
                firstId = false;
            } else assertTrue(id.getTemplateId().equalsIgnoreCase(ID_TEMP3));
        }
    ...
    void setUp(JenkinsRule j) {
        this.j = j;
        descriptor = (ExtendedEmailTemplatePublisher.DescriptorImpl)
                j.jenkins.getDescriptor(ExtendedEmailTemplatePublisher.class);
        descriptor.addTemplate(
                new ExtendedEmailPublisherTemplate(ID_TEMP1, NAME_TEMP1, "Desc", new ExtendedEmailPublisher()));
        descriptor.addTemplate(
                new ExtendedEmailPublisherTemplate(ID_TEMP2, NAME_TEMP2, "Desc", new ExtendedEmailPublisher()));
        descriptor.addTemplate(
                new ExtendedEmailPublisherTemplate(ID_TEMP3, NAME_TEMP3, "Desc", new ExtendedEmailPublisher()));
    ...
    private FreeStyleProject createProject(final String name, final List<TemplateId> templateIds) throws IOException {
